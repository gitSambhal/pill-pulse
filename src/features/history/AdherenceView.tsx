  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        {/* Adherence Card */}
        <div className="p-4 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 shadow-xs flex flex-col justify-between">
          ...
          <div>
            <div className="text-3xl font-bold tracking-tight text-teal-600 dark:text-teal-400">
              {adherencePercent}%
            </div>
          ...
          {past7Days.map((dStr) => {
            ...
            return (
              <button
                ...
                className="p-2.5 rounded-2xl border border-slate-200/70 dark:border-slate-800/70 hover:border-teal-500/60 hover:bg-teal-50/30 dark:hover:bg-teal-950/30 transition-all flex items-center justify-between text-xs text-left"
              >
                <div>
        ...
        <div className="space-y-2">
          {history.slice(0, 12).map((log) => {
            ...
            return (
              <div
                key={log.id}
                className="p-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/60 flex items-center justify-between text-xs"
              >
                <div className="flex items-center gap-2.5 min-w-0 pr-2">
                  <div className="w-7 h-7 rounded-full bg-teal-50 dark:bg-teal-950/60 text-teal-600 dark:text-teal-400 flex items-center justify-center shrink-0">
          ...
          {history.length === 0 && (
            <div className="text-center py-8 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 text-xs">
